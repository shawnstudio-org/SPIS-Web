                ...
                <div className="table-wrap">
                    <table className="table">
                        ...
                        <tbody>
                            {reports.map((r, i) => (
                                <tr key={r.id || i}>
                                    <td>{r.exam_date || r.created_at?.split('T')[0] || 'Unknown Date'}</td>
                                    <td>
                                        <span className="mono">{r.exam_name || 'Mixed'}</span>
                                    </td>
                                    <td>{r.score_total ?? '-'}</td>
                                    <td>
                                        <span className={`status-pill ${r.analysis_status === 'DONE' ? 'done' : 'pending'}`}>
                                            {r.analysis_status || r.entry_status || 'PENDING'}
                                        </span>
                                    </td>
                                    <td>
