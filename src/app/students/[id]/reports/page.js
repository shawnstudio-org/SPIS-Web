    ...
    >
      <div className="card">
        ...
        <div className="btn-row" style={{ marginBottom: 12 }}>
          <Link className="primary-btn" href={`/students/${studentId}/reports/new`}>Quick Analyze</Link>
          <Link className="secondary-btn" href={`/students/${studentId}/reports/new/advanced`}>Advanced Mapping</Link>
                ...
                <tr key={report.id}>
                  ...
                  <td>
                    <div className="form-grid" style={{ gap: 4 }}>
                      <span>{report.score_total ?? '-'} / {report.percentile ? `${report.percentile}%` : '-'}</span>
                      {report.rank_position && report.rank_total ? (
                        <span className="mono">Rank {report.rank_position}/{report.rank_total}</span>
                      ) : null}
                    </div>
                  </td>
                  <td>
