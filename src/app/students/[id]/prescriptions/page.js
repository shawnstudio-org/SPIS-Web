...
import { normalizeDiagnosis, normalizePrescription } from '@/lib/view-models';
...
export default function PrescriptionsPage({ params }) {
  ...
  async function generatePrescription() {
    ...
    if (!response.ok) {
      setError(getApiErrorMessage(response, 'Unable to generate prescription.'));
      return;
    }
  ...
  async function updateItemStatus(itemId, status) {
    ...
    if (!response.ok) {
      setError(getApiErrorMessage(response, 'Unable to update item status.'));
      return;
    }
  ...
  const latestDiagnosis = normalizeDiagnosis(diagnostics);
  const latestPrescription = normalizePrescription(prescription);
    ...
    >
      ...
      <div className="metric-grid">
        ...
        <MetricCard
          ...
          value={latestPrescription.expected_improvement_pct ? `${latestPrescription.expected_improvement_pct}%` : 'N/A'}
          hint="Projected gain from current action plan"
        />
        <MetricCard
          label="Repetitions / Week"
          value={latestPrescription.recommended_repetitions_per_week || 'N/A'}
          hint="Recommended average cadence"
        />
        <MetricCard
          label="Items"
          value={latestPrescription.items.length}
      ...
      <div className="card form-grid">
        ...
        {latestPrescription.empty ? (
          <EmptyState
            title="No prescription generated yet"
            description={latestDiagnosis.empty
              ? 'Run the first diagnosis before generating a prescription.'
              : 'Generate a prescription from the latest analysis to see recommended drills.'}
          />
        ) : (
          <>
            <p>{latestPrescription.summary}</p>
