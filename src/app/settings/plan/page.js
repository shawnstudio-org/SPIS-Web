...
export default function PlanSettingsPage() {
  ...
  async function loadPlan() {
    ...
    if (!response.ok) {
      setError(response.data?.error || 'Unable to load plan');
      return;
    }
  ...
  async function assignPlan(event) {
    ...
    if (!response.ok) {
      setError(response.data?.error || 'Plan assignment failed. Verify ADMIN_SECRET and user ID.');
      return;
    }
  ...
  async function redeemCoupon(event) {
    ...
    if (!response.ok) {
      setCouponError(response.data?.error || 'Coupon redemption failed');
      return;
    }
  ...
  return (
    <AppShell title="Plan Settings" user={auth.user} plan={auth.plan} token={auth.token}>
      ...
      <div className="card form-grid">
        ...
        <p>Billing is intentionally disabled. Assign plans manually via admin secret.</p>
